...
for k = 1 : Nf
    ...
    % l12 = 0.05;
    % rho = 0.5;
    l1  = 0.13; nu1  = 2.10; s1 = 0.66;
    l2  = 0.10; nu2  = 0.50; s2 = std(yfobs(:, k)); %s2 = 0.96;
    l12 = 0.12;
    rho = 0.8;
    
    % Model selection
    hyp0   = [log([l1; nu1; s1; l2; nu2; s2; l12]); log((rho + 1) / (1 - rho)); log(1e-3); log(1e-3)];
    nlml0k = gausslikm(hyp0, covfunc, {xcobs, xfobs}, {ycobs(:, k), yfobs(:, k)});
    tic
    try
        [hypok, nlmlok, statusk] = fmincon(@(h) gausslikm(h, covfunc, {xcobs, xfobs}, {ycobs(:, k), yfobs(:, k)}), hyp0, [], [], [], [], [], [], @mgp_cons, opts);
    catch
        continue
    end
...
% Output    
save(sprintf('%s_hypo_ML.mat', testID), 'hypo', 'nlml0', 'nlmlo', 'status');